...
export default function ShareLink({ shareLink, shareLoading, onShare, disabled }) {
  ...
  const buttonClassName = useCallback(() => {
    return `px-4 py-2 rounded-lg font-medium text-white transition-colors flex items-center space-x-2
      ${shareLoading || disabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#4F46E5] hover:bg-[#6a63e9]'}`;
  }, [shareLoading, disabled]);
  ...
  return (
    <div className='mt-4 flex justify-center'>
      <button onClick={onShare} disabled={shareLoading || disabled} className={buttonClassName()}>
