...
hf_datasets = ['shahxeebhassan/human_vs_ai_sentences', 'ardavey/human-ai-generated-text']

kaggle_datasets = [
    (
        'sunilthite/llm-detect-ai-generated-text-dataset',
        lambda df: (
            df[['text', 'generated']].assign(
...
def load_kaggle() -> Dataset:
    ...
    for dataset_name, preparation_func in kaggle_datasets:
        ...
        processed_datasets.append(Dataset.from_pandas(processed_df, preserve_index=False))
...
full_df['text_clean'] = full_df['text'].str.replace(r'\s+', ' ', regex=True).str.strip()
